...
FIRST_PAGE = 'https://www.80s.tw/ju/list'
START_PAGE = 'https://www.80s.tw/ju/list/----0--p'
PAGE_NUM = 1
PAGE_TOTAL = 130
WRITE_MONGODB = True
...
class Handler(BaseHandler):
    ...
    def index_page(self, response):
        for each in response.doc('.h3 > a').items():
            if each.attr.href.split('/')[-2:-1] == ['ju']:
    ...
    def detail_page(self, response):
        ...
        # 构建两块信息
        brief_info = format_brief_info(response, 'ju')
        ...
        if mark:
            if WRITE_MONGODB:
                download_json_final = get_download_info(response, 'ju', mark)
